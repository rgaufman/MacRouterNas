...
class BlacklistUpdater
  ...
  def update
    ...
    # Build complete whitelist (constant + user whitelist)
    whitelist = WHITELIST.dup
    if @whitelist_file && File.exist?(@whitelist_file)
      user_whitelist = File.readlines(@whitelist_file).map(&:strip).reject do |line|
  ...
  def download_and_parse(url)
    ...
    URI.open(url) do |file|
      file.each_line do |line|
        ...
        if domain&.include?('.') && !domain.start_with?('0.0.0.0') && !domain.start_with?('127.0.0.1')
          domains.add(domain)
        end
      end
    end
