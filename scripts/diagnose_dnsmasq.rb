...
process_check = shell('pgrep -l dnsmasq')
...
more_detailed_process = shell('ps aux | grep dnsmasq | grep -v grep')
...
port_check = shell('sudo lsof -i :67')
...
if port_check[:success] && !port_check[:stdout].empty?
  ...
  if lines.length > 1
    ...
    lines.each_with_index do |line, index|
      ...
      next unless index.positive? # Skip header

      columns = line.split
      puts "  - Process name: #{columns[0]}" if columns.length >= 1
      puts "  - PID: #{columns[1]}" if columns.length >= 2
      puts "  - Protocol: #{columns[7]}" if columns.length >= 8
      puts "  - Address:Port: #{columns[8]}" if columns.length >= 9
    end
  end
else
...
brew_services = shell('brew services list | grep dnsmasq')
...
launchd_check = shell('sudo launchctl list | grep dnsmasq')
...
custom_service = shell('sudo launchctl list | grep custom.dnsmasq')
...
launchdaemon_check = shell('ls -la /Library/LaunchDaemons/*dnsmasq*')
...
if File.exist?(config_path)
  ...
  file_perms = shell("ls -la #{config_path}")
  puts "File permissions: #{file_perms[:stdout]}"

  # Get non-comment lines from config
  config_content = shell("grep -v '^#' #{config_path} | grep -v '^$'")
