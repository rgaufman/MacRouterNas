...
# Update MSS clamping value from 1452 to 1440
pf_content.gsub!("scrub out on #{@wan} proto tcp all max-mss 1452",
                 "scrub out on #{@wan} proto tcp all max-mss 1440")

# Write the updated content back to the file
File.write(PF_MANAGER_FILE, pf_content)
puts "Updated MSS clamping value to 1440 in #{PF_MANAGER_FILE}"
...
template_content.gsub!('scrub out on <%= @wan_interface %> proto tcp all max-mss 1452',
                       'scrub out on <%= @wan_interface %> proto tcp all max-mss 1440')

# Write the updated content back to the file
File.write(TEMPLATE_FILE, template_content)
puts "Updated MSS clamping value to 1440 in #{TEMPLATE_FILE}"
